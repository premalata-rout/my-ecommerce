...
//... (RegisterPage, ProductDetail, Admin same as your code)
...
function Admin() {
  ...
  const handleSubmit = (e) => {
    ...
    fetch(url, { method, headers: { "Content-Type": "application/json" }, body: JSON.stringify({...form, price: Number(form.price) }) }).then(() => {
        alert(editingId? "Product Updated!" : "Product Added!"); setEditingId(null); setForm({ name: "", price: "", image: "", category: "Laptop", desc: "" }); fetchProducts();
    });
  };
  ...
  return (
    <div style={{ padding: "20px", maxWidth: "600px", margin: "auto" }}>
      ...
      {products.map((p) => (
        <div key={p._id || p.id} style={{ border: "1px solid #ccc", padding: "10px", margin: "10px 0", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <div><button onClick={() => handleEdit(p)} style={{ marginRight: "5px", background: "blue", color: "white", padding: "5px 10px" }}>Edit</button><button onClick={() => handleDelete(p._id || p.id)} style={{ background: "red", color: "white", padding: "5px 10px" }}>Delete</button></div>
        </div>
      ))}
    </div>
  );
