...
function MyOrders() {
  ...
  if (!userId) return <h2>Please Login to see orders!</h2>;
...
function LoginPage() {
  ...
  const login = async () => {
    ...
    if (res.ok) {
      ...
      localStorage.setItem("userEmail", data.user.email); // for MyOrders
      alert("Login Success");
      navigate("/");
...
function Admin() {
  ...
  if (!isAdmin) {
    return (
      <div style={{ padding: "50px", maxWidth: "400px", margin: "100px auto", border: "2px solid black", borderRadius: "10px", textAlign: "center" }}>
        <h2>🔒 Admin Login</h2>
        <input type="password" placeholder="Enter Password" value={pass} onChange={(e) => setPass(e.target.value)} style={{ padding: "10px", width: "90%", margin: "10px" }} /><br />
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (editingId) {
      fetch(`${API}/products/${editingId}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({...form, price: Number(form.price) }) }).then(() => {
        alert("Product Updated!"); setEditingId(null); setForm({ name: "", price: "", image: "", category: "Laptop", desc: "" }); fetchProducts();
      });
    } else {
      fetch(`${API}/products`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({...form, price: Number(form.price) }) }).then(() => {
        alert("Product Added!"); setForm({ name: "", price: "", image: "", category: "Laptop", desc: "" }); fetchProducts();
      });
    }
  };
  const handleEdit = (p) => { setForm({ name: p.name, price: p.price, image: p.image, category: p.category, desc: p.desc || "" }); setEditingId(p.id); window.scrollTo(0, 0); };
  ...
  return (
    <div style={{ padding: "20px", maxWidth: "600px", margin: "auto" }}>
      ...
      <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "15px" }}>
        ...
        <button type="submit" style={{ padding: "12px", background: editingId? "green" : "black", color: "white" }}>{editingId? "Update Product" : "Add Product"}</button>
      </form>
      <hr style={{ margin: "30px 0" }} /><h2>All Products ({products.length})</h2>
      {products.map((p) => (
        <div key={p.id} style={{ border: "1px solid #ccc", padding: "10px", margin: "10px 0", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div style={{ display: "flex", gap: "10px", alignItems: "center" }}><img src={p.image} width="50" height="50" style={{ objectFit: "cover" }} /><div><b>{p.name}</b><br />₹{p.price}</div></div>
          <div><button onClick={() => handleEdit(p)} style={{ marginRight: "5px", background: "blue", color: "white", padding: "5px 10px" }}>Edit</button><button onClick={() => handleDelete(p.id)} style={{ background: "red", color: "white", padding: "5px 10px" }}>Delete</button></div>
        </div>
      ))}
    </div>
  );
...
function App() {
  ...
  const addToCart = (product) => {
    ...
    if (exist) { setCart(cart.map((x) => getId(x) === pid? {...x, quantity: x.quantity + 1 } : x)); }
  ...
  // UPDATED placeOrder with Backend Call
  ...
  return (
    <BrowserRouter>
      <div style={{ padding: "20px", fontFamily: "Arial" }}>
        ...
        <Link to="/myorders"><button style={{background:"purple", color:"white"}}>My Orders</button></Link>
